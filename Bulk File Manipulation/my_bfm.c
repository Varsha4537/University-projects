...
// Function prototypes
void print_usage();
void create_file_or_directory(const char *filename, const char *operation, const char *arg);
...
void process_files_recursive(const char *dir_name, const char *filename, const char *arg, const char *operation);
void search_file_recursive(const char *dir_name, const char *operation, const char *filename, const char *arg);
void recursively_append_text(const char *dir_path, const char *filename, const char *text);
void recursively_append_even_numbers(const char *dir_path, const char *filename, int start);
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(operation, "-c") == 0) {
        ...
        if (argc >= 4) {
            ...
            printf("Error: New filename not provided for renaming.\n");
            return 1;
        }
        ...
        if (argc >= 4) {
            ...
            printf("Error: Text or start point not provided.\n");
            return 1;
        }
    } else {
        print_usage();
...
void print_first_50_bytes(const char *filename) {
    ...
    int byte;
    for (int i = 0; i < MAX_BYTES_TO_DISPLAY && (byte = fgetc(file)) != EOF; ++i) {
        printf("%d ", byte);
    }
    printf("\n");
    fclose(file);
    ...
    if ((dir = opendir(dir_path)) != NULL) {
        while ((ent = readdir(dir)) != NULL) {
            if (ent->d_type == DT_REG && strcmp(ent->d_name, filename) == 0) {
                ...
                if (file != NULL) {
                    append_text(file, text); // Pass the FILE* pointer to append_text function
    ...
    if ((dir = opendir(dir_path)) != NULL) {
        while ((ent = readdir(dir)) != NULL) {
            if (ent->d_type == DT_REG && strcmp(ent->d_name, filename) == 0) {
                char filepath[MAX_PATH_LENGTH];
                snprintf(filepath, MAX_PATH_LENGTH, "%s/%s", dir_path, filename);
                FILE *file = fopen(filepath, "ab"); // Open the file in binary append mode
                if (file != NULL) {
                    append_even_numbers(file, start); // Pass the FILE* pointer and the starting number to append_even_numbers function
